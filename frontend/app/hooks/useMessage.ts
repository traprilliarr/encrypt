...
export const useListenToNewMessage = (
    ...
    useEffect(() => {
        const newMessageHandler = (data: newMessageComesIn) => {
            let [privateKey, publicKey] = [loggedInUserKey.privateKey, deserializeECDHPublicKey(data.sender.public_key)];
            ...
            data.content = decryptConversation(new Uint8Array(message),
                new Uint8Array(sharedSecret), new Uint8Array(iv));
    ...
    }, [socketParam, socketParam.connected, user, loggedInUserKey.privateKey])
