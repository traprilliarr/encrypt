...
export const useConversationLogic = (
  ...
  const { encryptMessageForSending, decryptMessage } = useCrypto(otherUser.user.public_key)
  ...
  useEffect(() => {
    if (user) {
      (async () => {
        try {
          const messages = await getMessagesByConversationIdService(user.backend_token, conversationId);
    ...
    return () => { }
  }, [params.conversationId, router, setMessagesOnConversation, user])
