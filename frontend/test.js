...
const getPBKDF2KeyMaterial = async (password) => {
    const enc = new TextEncoder();
    return await crypto.subtle.importKey("raw", enc.encode(password), "PBKDF2", false, ["deriveBits", "deriveKey"]);
};
...
const deriveKeyFromPassword = async (password, keyLength) => {
    ...
    const pbkdf2DerivedKey = await crypto.subtle.deriveKey(pbkdf2Params, keyMaterial, {
        name: "AES-CBC",
        length: 256,
...
export const deriveBitsFromPassword = async (password, keyLength) => {
    ...
    const derivedBits = await crypto.subtle.deriveBits(pbkdf2Params, keyMaterial, keyLength * 8);
    return {
        algo,
        iter,
        key_size: keyLength * 8,
...
function coerceArray(arg, copy = false) {
    // ArrayBuffer view
