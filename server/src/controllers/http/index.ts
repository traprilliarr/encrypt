...
import addMessageToChat, { addMessageToChatSchema } from "./chats/add_message_to_chat.js";
import getChats from "./chats/get_chats.js";
import getMessagesByChat, { getMessagesByChatSchema } from "./chats/get_messages_by_chat.js";
...
const initRoutes = async (app: express.Application) => {
    ...
    protectedRoutes.use(asyncHandler(expressjwt({ secret: env.JWT_SECRET, algorithms: ["HS256"] })))

    protectedRoutes.get("/api/users", asyncHandler(getUsersExcludeMe));
    protectedRoutes.get("/api/user", asyncHandler(getLoggedInUser));
    protectedRoutes.get("/api/user/:id", ...validate['getUserById'], asyncHandler(getUserById));

    // @ts-ignore:next-line
    protectedRoutes.get("/api/chats", paginate.middleware(20, 100), asyncHandler(getChats));
    protectedRoutes.post("/api/chats", ...validate['addChat'], asyncHandler(addChat));

    protectedRoutes.get("/api/chats/:chat_id", validate['getChatById'], asyncHandler(getChatById));

    // @ts-ignore:next-line
    protectedRoutes.get("/api/chats/:chat_id/messages", paginate.middleware(20, 100), ...validate['getMessagesByChat'], asyncHandler(getMessagesByChat))
    protectedRoutes.post("/api/chats/:chat_id/messages", ...validate['addMessageToChat'], asyncHandler(addMessageToChat))
    protectedRoutes.post("/api/chats/:chat_id/messages/:message_id/seen", ...validate['seenMessage'], asyncHandler(seenMessage))
...
const asyncHandler: (fn: any) => express.RequestHandler = (fn) => (req, res, next) => {
    return Promise
        .resolve(fn(req, res, next))
        .catch(next);
};

const validateHasAccessToChat: express.RequestHandler = async (req: any, res, next) => {
...
const validate = {
    ...
    'getMessagesByChat': [validateHasAccessToChat, getMessagesByChatSchema, validationErrorMiddleware],
    'getChatById': [validateHasAccessToChat, validationErrorMiddleware],
    'addMessageToChat': [validateHasAccessToChat, addMessageToChatSchema, validationErrorMiddleware],
    'addChat': [addChatSchema, validationErrorMiddleware],
    'seenMessage': [validateHasAccessToChat, seenMessageSchema, validationErrorMiddleware]
}
